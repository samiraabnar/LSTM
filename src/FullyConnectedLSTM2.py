...
class FullyConnectedLSTM2(object):
    ...
    def build_model(self):
        ...
        E_value = np.asarray(
            self.random_state.uniform(-np.sqrt(1.0 / (self.embedding_dim+self.input_dim)),
                                      np.sqrt(1.0 / (self.embedding_dim+self.input_dim)),
                                      (self.embedding_dim,self.input_dim))
            , dtype=theano.config.floatX)
        self.E = theano.shared(value=E_value, name="E", borrow="True")

        def embed(x_t):
            x_e = self.E.dot(x_t).astype(theano.config.floatX)
            return x_e

        X_e, updates = theano.scan(embed,sequences=[x])
        ...
        for i in range(len(params)):
            updates[i] = (params[i], updates[i] / self.batch_size)
            ...
            for i in np.random.permutation(len(X_train)):
                ...
                cost = self.sgd_step(np.asarray(X_train[i], dtype=np.float32) * [dropout for i in
                    np.arange(len(X_train[i]))]
                                     ...
                                     #, np.asarray([pow(c,2) for c in np.arange(len(X_train[i]),0,-1)], dtype=np.int32) //dist
                                     )
    ...
    def train_1layer_glove_wordembedding(hidden_dim, modelfile):
        ...
        binary_embedded_dev = []
        binary_dev_labels = []
        for i in np.arange(len(embedded_dev)):
            if np.argmax(dev_labels[i]) != 1:
                binary_embedded_dev.append(embedded_dev[i])
                binary_dev_labels.append(np.eye(2)[np.argmax(dev_labels[i]) // 2])
